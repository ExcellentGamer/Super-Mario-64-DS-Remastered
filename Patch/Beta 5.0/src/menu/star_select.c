...
void render_100_coin_star(u8 stars) {
    if (stars & (1 << INDEX_STAR_100_COINS)) {
        // If the 100 coin star has been collected, create a new star selector next to the coin score.
        sStarSelectorModels[INDEX_STAR_100_COINS] = spawn_object_abs_with_rot(gCurrentObject, 0, MODEL_STAR,
                                                        bhvActSelectorStarType, 370, 24, -300, 0, 0, 0);
        sStarSelectorModels[INDEX_STAR_100_COINS]->oStarSelectorSize = 0.8;
        sStarSelectorModels[INDEX_STAR_100_COINS]->oStarSelectorType = STAR_SELECTOR_100_COINS;
    }
}
...
void print_act_selector_strings(void) {
    ...
    if (sVisibleStars != 0) {
        selectedActName = segmented_to_virtual(actNameTbl[(gCurrCourseNum - 1) * NUM_ACTS_PER_COURSE + sSelectedActIndex]);
    ...
    if (sActSelectorMenuTimer >= 11) {
        ...
        if ((gPlayer3Controller->buttonPressed & A_BUTTON)
         ...
         || (gPlayer3Controller->buttonPressed & B_BUTTON)) {
            r96_play_character_sound_no_arg(R96_MARIO_SOUND_LETS_A_GO, R96_LUIGI_SOUND_LETS_A_GO, R96_WARIO_SOUND_LETS_A_GO, R96_WALUIGI_SOUND_LETS_A_GO, R96_YOSHI_DS_SOUND_LETS_A_GO);
