...
#define SAVE_FILE_WALUIGI_SOUND R96_WALUIGI_OKEY_DOKEY
#define SAVE_FILE_YOSHI_DS_SOUND R96_YOSHI_DS_OKEY_DOKEY
...
char *menu_sound(s8 currentSave){
        switch(save_file_get_player_model(currentSave)){
            ...
            case 0 :
                return SAVE_FILE_YOSHI_DS_SOUND;
                break;
            case 1 :
            ...
            case 2 :
                ...
                break;
            case 3 :
                return SAVE_FILE_WALUIGI_SOUND;
                break;
            default :
...
void print_save_file_star_count(s8 fileIndex, s16 x, s16 y) {
    ...
    if (save_file_exists(fileIndex) == TRUE) {
        ...
        // Print "new" text
        print_hud_lut_string(HUD_LUT_GLOBAL, x, y, get_key_string("TEXT_NEW"));
    }
}
...
void print_erase_menu_prompt(s16 x, s16 y) {
    ...
    if (sCursorClickingTimer == 2) {
        ...
        if (sEraseYesNoHoverState == MENU_ERASE_HOVER_YES) {
            r96_play_character_sound_no_arg(R96_MARIO_FALLING, R96_LUIGI_FALLING, R96_WARIO_FALLING, R96_WALUIGI_FALLING, R96_YOSHI_DS_FALLING);
    ...
    print_generic_string(x + 56, y, get_key_string("TEXT_YES"));
    gDPSetEnvColor(gDisplayListHead++, sYesNoColor[1], sYesNoColor[1], sYesNoColor[1], sTextBaseAlpha);
    print_generic_string(x + 98, y, get_key_string("TEXT_NO"));
    gSPDisplayList(gDisplayListHead++, dl_ia_text_end);
}
...
void print_score_file_castle_secret_stars(s8 fileIndex, s16 x, s16 y) {
    ...
    // Print "[star] x"
    print_menu_generic_string(x, y, get_key_string("TEXT_STAR_X"));
...
void print_score_file_course_coin_score(s8 fileIndex, s16 courseIndex, s16 x, s16 y) {
    ...
    if (sScoreFileCoinScoreMode == 0) {
        // Print "[coin] x"
        print_menu_generic_string(x + 25, y, get_key_string("TEXT_COIN_X"));
        // Print coin score
        ...
        if (stars & (1 << 6)) {
            print_menu_generic_string(x + 70, y, get_key_string("TEXT_STAR"));
        }
    }
    // HISCORE
    else {
        // Print "[coin] x"
        print_menu_generic_string(x + HISCORE_COIN_ICON_X, y, get_key_string("TEXT_COIN_X"));
        ...
        print_menu_generic_string(x + HISCORE_COIN_NAMES_X, y, fileNames[(save_file_get_max_coin_score(courseIndex) >> 16) & 0xFFFF]);
    }
}
 ...
 */
 void print_save_file_scores(s8 fileIndex) {
    ...
    gSPDisplayList(gDisplayListHead++, dl_rgba16_text_begin);
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
    print_hud_lut_string(HUD_LUT_DIFF, MARIO_X, 15, get_key_string("TEXT_MARIO"));
...
#undef PADCHAR
    ...
    if (sScoreFileCoinScoreMode == 0) {
        print_menu_generic_string(MYSCORE_X, 24, get_key_string("TEXT_MY_SCORE"));
    } else {
        print_menu_generic_string(HISCORE_X, 24, get_key_string("TEXT_HUD_HI_SCORE"));
    }
    gSPDisplayList(gDisplayListHead++, dl_menu_ia8_text_end);
}
