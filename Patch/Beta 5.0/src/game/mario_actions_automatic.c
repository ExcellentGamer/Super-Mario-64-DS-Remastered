...
s32 act_grab_pole_slow(struct MarioState *m) {
    r96_play_character_sound_if_no_flag(m, R96_MARIO_WHOA, R96_LUIGI_WHOA, R96_WARIO_WHOA, R96_WALUIGI_WHOA, R96_YOSHI_DS_WHOA, MARIO_MARIO_SOUND_PLAYED);
    if (set_pole_position(m, 0.0f) == POLE_NONE) {
...
s32 act_hang_moving(struct MarioState *m) {
    ...
    if (m->marioObj->header.gfx.curAnim.animFrame == 12) {
...
s32 act_ledge_climb_slow(struct MarioState *m) {
    ...
    if (m->actionTimer == 10) {
        r96_play_character_sound_if_no_flag(m, R96_MARIO_PULL_UP, R96_LUIGI_PULL_UP, R96_WARIO_PULL_UP, R96_WALUIGI_PULL_UP, R96_YOSHI_DS_PULL_UP, MARIO_MARIO_SOUND_PLAYED);
    }

    update_ledge_climb(m, MARIO_ANIM_SLOW_LEDGE_GRAB, ACT_IDLE);

    update_ledge_climb_camera(m);
    if (m->marioObj->header.gfx.curAnim.animFrame == 17) {
...
s32 act_ledge_climb_fast(struct MarioState *m) {
    ...
    if (m->marioObj->header.gfx.curAnim.animFrame == 8) {
