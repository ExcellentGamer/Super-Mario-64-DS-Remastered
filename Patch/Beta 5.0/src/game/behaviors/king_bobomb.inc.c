...
void king_bobomb_act_throw_bobomb(void) { //throw bobomb here I guess
    //play animation and somehow add a bobomb model to king bobomb's hands (note: common_anchor_mario_behavior)
    //wait a couple of frames
    //spawn a bobomb, disable it's respawner and give it a velocity
}
...
void king_bobomb_act_0(void) { // act bobomb before fight
    o->oForwardVel = 0;
    o->oVelY = 0;
    if (o->oSubAction == 0) {
...
void king_bobomb_act_3(void) { // mario grabbed
    if (o->oSubAction == 0) {
        o->oForwardVel = 0;
...
void king_bobomb_act_7(void) { // death dialogue
    ...
    if (cur_obj_update_dialog_with_cutscene(2, 2, CUTSCENE_DIALOG, DIALOG_116)) {
        ...
        cur_obj_shake_screen(SHAKE_POS_SMALL);
        cur_obj_spawn_star_at_y_offset(3070.0f, 3347.0f, -4707.0f, 200.0f);
        o->oAction = 8;
    }
}

void king_bobomb_act_8(void) { // dies
    if (o->oTimer == 60){
        r96_stop_music();
        r96_cap_music_boss_fix();
        //dynos_music_pause();
...
void (*sKingBobombActions[])(void) = {
    ...
    king_bobomb_act_5, king_bobomb_act_6, king_bobomb_act_7, king_bobomb_act_8, king_bobomb_act_throw_bobomb,
};
