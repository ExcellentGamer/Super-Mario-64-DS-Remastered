...
void mario_blow_off_cap(struct MarioState *m, f32 capSpeed) {
    ...
    if (does_mario_have_hat(m)) {
		...
		else if(!isLuigi() && !isWario() && !isWaluigi() && !isYoshi())
...
u32 interact_coin(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    if (COURSE_IS_MAIN_COURSE(gCurrCourseNum) && m->numCoins - o->oDamageOrCoinValue < 100
        && m->numCoins >= 100) {
        bhv_spawn_star_no_level_exit(INDEX_STAR_100_COINS);
    }
    ...
    if (o->oInteractionSubtype & INT_SUBTYPE_FADING_WARP) {
        ...
        if (m->action != ACT_EMERGE_FROM_PIPE) {
			...
			if(o->oObjectID == 1){
				...
				if(m->numKeys >= 1) {
                    if (warpid == 3){
                        characterModel = getCharacterType() == MARIO_DEFAULT ? WALUIGI : WALUIGI;
                        animation = set_mario_action(m, ACT_CHARACTER_SWITCH, TRUE);
                    }
                }
    ...
    if (m->action == ACT_WALKING || m->action == ACT_DECELERATING) {
        ...
        if (m->action == ACT_WALKING || m->action == ACT_DECELERATING) {
            ...
            if (doorAction == 0) {
                ...
                if (warpDoorId==3 && gLoadedGraphNodes[MODEL_PLAYER] == gLoadedGraphNodes[MODEL_MARIO])
                {
                    m->doorType=1;
                }else if (warpDoorId==3)
                {
                    m->doorType=4;
                }
                if (warpDoorId==4 && gLoadedGraphNodes[MODEL_PLAYER] == gLoadedGraphNodes[MODEL_LUIGI])
                {
                    m->doorType=2;
                }else if (warpDoorId==4)
                {
                    m->doorType=4;
                }
                if (warpDoorId==5 && gLoadedGraphNodes[MODEL_PLAYER] == gLoadedGraphNodes[MODEL_WARIO])
                {
                    m->doorType=3;
                }else if (warpDoorId==5)
                {
                    m->doorType=4;
                }       
            }
...
u32 interact_bully(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    if (interaction & INT_ATTACK_NOT_FROM_BELOW) {
        ...
        update_mario_sound_and_camera(m);
        r96_play_character_sound(m, R96_MARIO_PULL_UP, R96_LUIGI_PULL_UP, R96_WARIO_PULL_UP, R96_WALUIGI_PULL_UP, R96_YOSHI_DS_PULL_UP);
    ...
    if (interaction & INT_ANY_ATTACK) {
        ...
        if (interaction & INT_HIT_FROM_ABOVE) {
            if (o->oInteractionSubtype & INT_SUBTYPE_TWIRL_BOUNCE) {
                ...
                reset_mario_pitch(m);
                r96_play_character_sound(m, R96_MARIO_TWIRL_BOUNCE, R96_LUIGI_TWIRL_BOUNCE, R96_WARIO_TWIRL_BOUNCE, R96_WALUIGI_TWIRL_BOUNCE, R96_YOSHI_DS_TWIRL_BOUNCE);
