...
void set_mario_initial_action(struct MarioState *m, u32 spawnType, u32 actionArg) {
    ...
    if(m->doorType==1)
    {
        gLoadedGraphNodes[MODEL_PLAYER] == gLoadedGraphNodes[MODEL_MARIO];
    }
    if(m->doorType==2)
    {
        gLoadedGraphNodes[MODEL_PLAYER] == gLoadedGraphNodes[MODEL_LUIGI];	
    }
    if(m->doorType==3)
    {
        gLoadedGraphNodes[MODEL_PLAYER] == gLoadedGraphNodes[MODEL_WARIO];
    }
    if(m->doorType==4)
    {
        gLoadedGraphNodes[MODEL_PLAYER] == gLoadedGraphNodes[MODEL_YOSHI_DS];
    }
...
void update_hud_values(void) {
    if (gCurrCreditsEntry == NULL) {
        ...
        gHudDisplay.wario_key = gMarioState->WarioKey;
...
s32 lvl_set_current_level(UNUSED s16 arg0, s32 levelNum) {
	...
	if (gCurrLevelNum == LEVEL_REC_ROOM) return 0;
...
s32 lvl_play_the_end_screen_sound(UNUSED s16 arg0, UNUSED s32 arg1) {
    if (save_file_get_total_star_count(gCurrSaveFileNum - 1, 0, 24) < 70) {
        ...
        r96_play_character_sound_no_arg(R96_MARIO_THANK_YOU_PLAYING_MY_GAME, R96_LUIGI_THANK_YOU_PLAYING_MY_GAME, R96_WARIO_THANK_YOU_PLAYING_MY_GAME, R96_WALUIGI_THANK_YOU_PLAYING_MY_GAME, R96_YOSHI_DS_THANK_YOU_PLAYING_MY_GAME);
    }
    return 1;
}
