...
static void cheats_play_as_set_model_and_anims(struct MarioState *m, s32 modelId, const void *anim) {
    ...
    if (modelId == MODEL_PLAYER) {
        // Todo give Waluigi his own animation set
        if (isLuigi() || isWaluigi())      gMarioState->animation = (struct MarioAnimation *) &Data_LuigiAnims;
...
s32 cheats_play_as(struct MarioState *m) {
    if (Cheats.EnableCheats) {
        ...
        switch(playAsIndex) {
            ...
            case 1: cheats_play_as_set_model_and_anims(m, MODEL_BLACK_BOBOMB, bobomb_anims[0]); break;
            case 2: cheats_play_as_set_model_and_anims(m, MODEL_BOBOMB_BUDDY, bobomb_buddy_anims[0]); break;
...
static const struct SpambaData sSpambaData[] = {
    ...
    { bhvBobomb,                    bobomb_geo,       "BOBOMB",                       0,   40, 300, 0x00000000, 0, 0 }, // SPAMBA_BOBOMB,
                ...
                switch (effect->index) {
                    ...
                    case CHAOS_BOBOMB_RAIN: {
                        if (effect->timer-- == 0) {
                            ...
                            bobomb->oAnimations = (struct Animation **) bobomb_anims;
