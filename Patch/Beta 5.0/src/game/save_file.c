...
u32 save_file_get_star_flags(s32 fileIndex, s32 courseIndex) {
    ...
    if (courseIndex == -1) {
        starFlags = (gSaveBuffer.files[fileIndex][0].flags >> 24) & 0xFF;
    } else {
...
s32 save_file_taken_wario_key(s32 fileIndex, s32 coinId){
    return gSaveBuffer.files[fileIndex][0].courseWarioKey[coinId];
}

void save_file_register_wario_key(s32 fileIndex, s32 coinId) {
    gSaveBuffer.files[fileIndex][0].courseWarioKey[coinId] = TRUE;

    gSaveFileModified = TRUE;
    save_file_do_save(fileIndex);
}

s32 save_file_get_wario_key(s32 fileIndex) {
    s32 keyAmount = 0;

    for(s32 tmp = 0; tmp < 6; tmp++){
        if(gSaveBuffer.files[fileIndex][0].courseWarioKey[tmp]) keyAmount++;
    }
