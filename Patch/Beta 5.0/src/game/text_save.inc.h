...
static s32 write_text_save(s32 fileIndex) {
    ...
    char wario_key[WARIO_KEY+1] = "";

    for (int id = 0; id < WARIO_KEY; id++) {
        sprintf(&wario_key[strlen(wario_key)], "%d", save_file_taken_wario_key(fileIndex, id));
    }    
...
static s32 read_text_save(s32 fileIndex) {
    ...
    if (value) {
        sscanf(value, "%s", wario_key);
        for(i = 0; i < WARIO_KEY; i++){
            gSaveBuffer.files[fileIndex][0].courseWarioKey[i] = (wario_key[i] == '1');
        }
    }   
