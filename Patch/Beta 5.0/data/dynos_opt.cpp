...
static s32 DynOS_Opt_ProcessInput(DynosOption *aOpt, s32 input) {
    switch (aOpt->mType) {
        ...
        case DOPT_CHOICESTAR:
            if (input == INPUT_LEFT) {
                *aOpt->mChoice.mIndex = (*aOpt->mChoice.mIndex + (NUM_ACTS_PER_COURSE - 1)) % (NUM_ACTS_PER_COURSE);
                return RESULT_OK;
            }
            if (input == INPUT_RIGHT || input == INPUT_A) {
                *aOpt->mChoice.mIndex = (*aOpt->mChoice.mIndex + 1) % (NUM_ACTS_PER_COURSE);
                return RESULT_OK;
            }
            break;
...
static void DynOS_Opt_CreateWarpToLevelSubMenu() {
    ...
    *aOpt->mChoice.mIndex = 0;
    ...
    // Area select
    {
    DynosOption *aOpt = DynOS_Opt_NewOption("dynos_warp_area", "", LABEL_NAME("Area Select"), "");
