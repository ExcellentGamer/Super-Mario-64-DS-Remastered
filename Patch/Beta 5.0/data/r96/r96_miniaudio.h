...
#include "include/stb_vorbis.c"    // Enables Vorbis decoding.
...
MA_API ma_reverb_node_config ma_reverb_node_config_init(ma_uint32 channels, ma_uint32 sampleRate, float roomSize, float damping, float width, float wet, float dry, float mode)
{
...
MA_API ma_result ma_reverb_node_init(ma_node_graph* pNodeGraph, const ma_reverb_node_config* pConfig, const ma_allocation_callbacks* pAllocationCallbacks, ma_reverb_node* pReverbNode)
{
    ...
    if (verblib_initialize(&pReverbNode->reverb, (unsigned long)pConfig->sampleRate, (unsigned int)pConfig->channels, pConfig->roomSize, pConfig->damping, pConfig->width, pConfig->wetVolume, pConfig->dryVolume, pConfig->mode) == 0) {
        return MA_INVALID_ARGS;
    }
