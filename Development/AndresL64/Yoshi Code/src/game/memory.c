...
static struct LuigiAnimDmaRelatedThing *func_luigiDMAunk(u8 *srcAddr) {
    struct LuigiAnimDmaRelatedThing *sp1C = dynamic_dma_read(srcAddr, srcAddr + sizeof(u32),
                                                             MEMORY_POOL_LEFT);
    u32 size = sizeof(u32) + (sizeof(u8 *) - sizeof(u32)) + sizeof(u8 *) +
               sp1C->count * sizeof(struct OffsetSizePair);
    main_pool_free(sp1C);
...
void func_luigiunk(struct LuigiAnimation *a, void *b, struct Animation *target) {
    if (b != NULL) {
        a->animDmaTable = func_luigiDMAunk(b);
    }
    a->currentAnimAddr = NULL;
    a->targetAnim = target;
}

s32 load_patchable_luigi_table(struct LuigiAnimation *a, u32 index) {
    s32 ret = FALSE;
    struct LuigiAnimDmaRelatedThing *sp20 = a->animDmaTable;
    u8 *addr;
    u32 size;
